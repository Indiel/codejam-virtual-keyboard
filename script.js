...
const arrKeys = Array.from(mapKeys.keys());
const keyboardRows = [[0, 14], [14, 29], [29, 42], [42, 55], [55, 64]];
...
function fillRow(length) {
  ...
  for (let i = length[0]; i < length[1]; i += 1) {
  ...
  if (target) {
    ...
    switch (target.dataset.key) {
      ...
      case 'AltRight':
        ...
        break;
      case 'Delete': {
        const text = textarea.textContent.slice(textarea.selectionStart
          + 1, textarea.textContent.length);
